				   ...
				   struct con_win *cwin)
{
	...
	if (file) {
		...
		if (is_playable_file(ab_file)) {
			...
			CDEBUG("(%s): Add file from file_tree: %s", __func__, ab_file);
		}
		g_free(ab_file);
		g_free(file);
		g_free(f_file);
			  ...
			  struct con_win *cwin)
{
	...
	if (bname) {
		...
		if (is_base_dir_and_accessible(fname, cwin)) {
			...
			if (!dir) {
				g_critical("(%s): Unable to open library : %s",
					   __func__, ab_dir);
				g_error_free(error);
				return;
			}
			...
			while (next_file) {
				...
				if (!g_file_test(ab_file, G_FILE_TEST_IS_DIR)) {
					if (is_playable_file(ab_file)) {
						CDEBUG("(%s): Play file from file_tree: %s",
						       __func__,
						       ab_file);
		       ...
		       struct con_win *cwin)
{
	...
	case TARGET_FILENAME:
		...
		CDEBUG("(%s): Fill DnD data, selection: %p, file_arr: %p",
...
void populate_file_tree(const gchar *new_dir, struct con_win *cwin)
{
	...
	if (!dir) {
		g_critical("(%s): Unable to open directory: %s", __func__, ab_new_dir);
		goto exit;
	}
...
void update_file_tree(gchar *new_dir, struct con_win *cwin)
{
	...
	if (!f_dir) {
		g_warning("(%s): Unable to get filename from UTF-8 string: %s\n",
			  __func__, new_dir);
		g_error_free(error);
		error = NULL;
		return;
				...
				struct con_win *cwin)
{
	...
	if (pgnum == PAGE_FILE) {
		...
		g_signal_handler_disconnect(G_OBJECT(cwin->browse_mode), switch_cb_id);
	}
}
