...
static gchar *license = "This program is free software: you can redistribute it and/or modify\n\
it under the terms of the GNU General Public License as published by\n\
the Free Software Foundation, either version 3 of the License, or\n\
(at your option) any later version.\n\n\
This program is distributed in the hope that it will be useful,\n\
...
static GtkWidget* lib_progress_bar(struct con_win *cwin, int update)
{
	...
	gtk_widget_set_size_request(progress_bar,
				    PROGRESS_BAR_WIDTH,
				    -1);
	gtk_button_box_set_layout(GTK_BUTTON_BOX(GTK_DIALOG(library_dialog)->action_area),
...
void rescan_library_action(GtkAction *action, struct con_win *cwin)
{
	...
	if (!cwin->cpref->library_dir) {
		g_warning("(%s): Library is not set, flushing existing library", __func__);
		flush_db(cwin);
		init_library_view(cwin);
		return ;
	}

	/* Check if versions are incompatible, if so drop tables and initialize schema */
	/* Otherwise, just flush the library database */

	if (is_incompatible_upgrade(cwin)) {
		if (drop_dbase_schema(cwin) == -1) {
			g_critical("(%s): Unable to drop database schema", __func__);
			return;
		}
		if (init_dbase_schema(cwin) == -1) {
			g_critical("(%s): Unable to init database schema", __func__);
			return;
		}
	} else {
		flush_db(cwin);
...
void add_all_action(GtkAction *action, struct con_win *cwin)
{
	...
	if (exec_sqlite_query(query, cwin, &result)) {
		for_each_result_row(result, i) {
			...
			if (!mobj)
				g_warning("(%s): Unable to retrieve details for location_id : %d",
