...
static GdkPixbuf* get_image_from_dir(gchar *path, struct con_win *cwin)
{
	...
	if (!dir) {
		g_critical("%s: Unable to open dir: %s\n", __func__, path);
		g_error_free(error);
		return NULL;
	}
	...
	while (next_file) {
		...
		if (g_file_test(ab_file, G_FILE_TEST_IS_REGULAR) &&
			...
			image = gdk_pixbuf_new_from_file_at_scale(ab_file,
								  ...
								  &error);
			if (!image) {
				g_critical("%s: Unable to open image file: %s\n",
					   __func__, ab_file);
				g_error_free(error);
			}
			g_free(ab_file);
...
void __update_current_song_info(struct con_win *cwin, gint length)
{
	...
	if (!cwin->cstate->curr_mobj) {
		g_critical("(%s): Curr mobj is invalid", __func__);
		return;
	}
	...
	if (cwin->cpref->show_album_art) {
		...
		if (mobj) {
			...
			if (album_art) {
				...
				cwin->album_art = gtk_image_new_from_stock(GTK_STOCK_INFO,
...
void shuffle_button_handler(GtkToggleButton *button, struct con_win *cwin)
{
	...
	current_playlist_clear_dirty_all(cwin);
 	...
 	if (!cwin->cpref->shuffle) {
		CDEBUG("(%s): Turning shuffle off", __func__);
		...
		CDEBUG("(%s): Turning shuffle on", __func__);
...
{
	CDEBUG("(%s): Repeat handler", __func__);
