...
static gint convert_samples(struct mad_pcm mpcm, guchar *out_buf)
{
	...
	if (nchannels > 2) {
		g_critical("(%s): No Support for channels > 2", __func__);
		return -1;
	}
			      ...
			      gsize *bytes_read)
{
	...
	if (ret == G_IO_STATUS_ERROR) {
		g_critical("(%s): Unable to read chars", __func__);
		ret = -1;
	}
	else if (ret == G_IO_STATUS_EOF) {
		g_warning("(%s): EOF reached", __func__);
		ret = -2;
	}
...
static void update_gui(struct con_mad_decoder *mdec, struct con_win *cwin)
{
	...
	if ((newsec > oldsec) && (newsec <= cwin->cstate->curr_mobj->tags->length)) {
				  ...
				  struct con_win *cwin)
{
	...
	if ((newsec > oldsec) && (newsec <= cwin->cstate->curr_mobj->tags->length)) {
		pbar_pos = newsec * (1/(gdouble)cwin->cstate->curr_mobj->tags->length);
		mdec->vbr_mmap_pos = g_array_append_val(mdec->vbr_mmap_pos, mmap_pos);
		mdec->vbr_pbar_pos = g_array_append_val(mdec->vbr_pbar_pos, pbar_pos);
		mdec->vbr_time_pos = g_array_append_val(mdec->vbr_time_pos, mdec->timer);
		mdec->vbr_seek_pos++;
	}
}
...
static gint mad_get_type(struct con_mad_decoder *mdec, struct con_win *cwin)
{
	...
	while (1) {
		if (mad_header_decode(&header, &stream)) {
			if (MAD_RECOVERABLE(stream.error))
				...
				g_critical("(%s): %s",
					   __func__, mad_stream_errorstr(&stream));
				break;
			}
		}
	...
	if (g_io_channel_seek_position(mdec->chan,
		...
		g_critical("(%s): Unable to seek to : %d", __func__, 0);
...
static gint cbr_seek(gint seek_len, struct con_mad_decoder *mdec, struct con_win *cwin)
{
	...
	frame_size = (mdec->no_samples * mdec->mframe.header.bitrate) / (8 *mdec->mframe.header.samplerate);
	...
	if (g_io_channel_seek_position(mdec->chan,
				       ...
				       G_SEEK_SET,
				       &err) != G_IO_STATUS_NORMAL) {
		g_critical("(%s): Unable to seek to : %d", __func__, seek_pos);
		return -1;
	}
		     ...
		     struct con_win *cwin)
{
	...
	old_fraction = gtk_progress_bar_get_fraction(GTK_PROGRESS_BAR(cwin->track_progress_bar));
	...
	if (seek_fraction < old_fraction) {
		...
		b_time = mad_timer_count(g_array_index(mdec->vbr_time_pos,
						       ...
						       tab_pos),
					 MAD_UNITS_SECONDS);
		read_start = (guchar*)g_mapped_file_get_contents(mdec->map) + b_mmap_pos;
	...
	while (1)
	{
		...
		if (mad_header_decode(&mdec->mheader, &mdec->mstream)) {
			if (MAD_RECOVERABLE(mdec->mstream.error)) {
				g_warning("(%s): %s",
					  __func__, mad_stream_errorstr(&mdec->mstream));
				continue;
			}
			else {
				if (mdec->mstream.error == MAD_ERROR_BUFLEN) {
					g_critical("(%s): %s",
						   __func__,
						   mad_stream_errorstr(&mdec->mstream));
					break;
				}
				else {
					g_critical("(%s): %s",
...
static gint mad_seek(struct con_mad_decoder *mdec, struct con_win *cwin)
{
	...
	if (mdec->mp3_type == CBR) {
		CDEBUG("(%s): CBR Seek", __func__);
		ret = cbr_seek(seek_len, mdec, cwin);
	}
	else {
		CDEBUG("(%s): VBR Seek", __func__);
...
static gint mad_decode(struct con_mad_decoder *mdec, struct con_win *cwin)
{
	...
	gint rem = 0, read_size = 0, ret = 0, cmd = 0;
	...
	do {
		...
		if ((mdec->mstream.buffer == NULL) ||
			...
			if (mdec->mstream.next_frame) {
				rem = mdec->mstream.bufend - mdec->mstream.next_frame;
				memmove(mdec->in_buf, mdec->mstream.next_frame, rem);
		...
		if (convert_samples(mdec->msynth.pcm, mdec->out_buf) < 0)
			g_critical("(%s): Unable to convert samples", __func__);
...
void play_mp3(struct con_win *cwin)
{
	...
	if (g_io_channel_set_encoding(mdec.chan, NULL, &err) != G_IO_STATUS_NORMAL) {
		g_critical("(%s): Unable to set IO Channel encoding", __func__);
		g_error_free(err);
		err = NULL;
		goto exit;
	}
	...
	if (mdec.mp3_type == VBR) {
		...
		mdec.map = g_mapped_file_new(cwin->cstate->curr_mobj->file, FALSE, &err);
		if (!mdec.map) {
			g_critical("(%s): MMAP Error : %s", __func__, err->message);
			g_error_free(err);
			err = NULL;
			goto exit;
		}
		CDEBUG("(%s): MMapped file: %s", __func__, cwin->cstate->curr_mobj->file);
	}
...
exit:
	if (g_io_channel_shutdown(mdec.chan, FALSE, &err) != G_IO_STATUS_NORMAL) {
		g_critical("(%s): IO Shutdown error : %s", __func__, err->message);
		g_error_free(err);
	}
	...
	if (mdec.mp3_type == VBR) {
		...
		if (mdec.map) {
			CDEBUG("(%s): Freeing mmaped file: %s",
			       __func__, cwin->cstate->curr_mobj->file);
