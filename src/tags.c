...
gboolean get_wav_info(gchar *file, struct tags *tags)
{
	...
	if (!sfile) {
		g_warning("(%s): Unable to open file using sndfile : %s",
			  __func__, file);
		return FALSE;
	}
	...
	if (sf_close(sfile))
		g_warning("(%s): Unable to close file using sndfile : %s",
			  __func__, file);
...
static gboolean get_info_taglib(gchar *file, struct tags *tags)
{
	...
	if (!tfile) {
		g_warning("(%s): Unable to open file using taglib : %s",
			  __func__, file);
		ret = FALSE;
		goto exit;
	}
	...
	if (!tag) {
		g_warning("(%s): Unable to locate tag", __func__);
		ret = FALSE;
		goto exit;
	}
	...
	if (!audio_prop) {
		g_warning("(%s): Unable to locate audio properties", __func__);
		ret = FALSE;
		goto exit;
	}
			   ...
			   int changed, struct con_win *cwin)
{
	...
	CDEBUG("(%s): Saving tags for file: %s", __func__, file);

	if (!taglib_file_save(tfile)) {
		g_warning("(%s): Unable to save tags for: %s\n", __func__, file);
		ret = FALSE;
	}
exit:
	taglib_file_free(tfile);
		...
		struct con_win *cwin)
{
	...
	CDEBUG("(%s): Tags Changed: 0x%x", __func__, changed);
	...
	/* Check if user is trying to set the same title/track no for multiple tracks */
	if (changed & TAG_TITLE_CHANGED) {
		if (loc_arr->len > 1) {
...
{
	GtkWidget *dialog;
	GtkWidget *label_tno, *label_title, *label_artist, *label_album, *label_genre, *label_year;
	GtkWidget *chk_tno, *chk_title, *chk_artist, *chk_album, *chk_genre, *chk_year;
	GtkWidget *entry_tno, *entry_title, *entry_artist, *entry_album, *entry_genre, *entry_year;
