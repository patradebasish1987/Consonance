...
gboolean is_base_dir_and_accessible(gchar *file, struct con_win *cwin)
{
	...
	absolute_name = g_strdup_printf("%s/%s", cwin->cstate->file_tree_pwd, file);
	if (g_file_test(absolute_name,
			G_FILE_TEST_IS_DIR) && !g_access(absolute_name, R_OK | X_OK))
		ret = TRUE;
	else
		ret = FALSE;
...
gint dir_file_count(gchar *dir_name, gint call_recur)
{
	...
	if (!dir) {
		g_warning("(%s): Unable to open library : %s", __func__, dir_name);
		return file_count;
	}
